...
int raghav(pixel2_t fileimage[]) {

long   nx =0, ny=0;        

char filename[20]="/data/edu/right.uyvy";
...
int y,y1,u,v;
		 ...
		 for (i=0; i<nx; i++)
		   {
			 ...
			 //y1 = (int)fileimage[nx*j+i].y2;
			 //u = (int)fileimage[nx*j+i].u;
			 //v = (int)fileimage[nx*j+i].v;
			//printf("y1: %d  u : %d y2 : %d v : %d\n",y,u,y1,v);
	...
	//y_avg = (int)(100*(y_avg/hits) / 80.0);
        printf("x average is %f\n", x_avg);
	//printf("y average is %f\n", y_avg);
    }
    return x_avg;
	}
...
int FEAT_OF_ACTIVE 	= 0; 	//optical flow results are read by control code, input into SIMULINK model and used for velocity estimation
int FEAT_POSVIS_RUN	= 0; 	//enable image processing computations and input them into SIMULINK model to record results
...
void RSEDU_control(HAL_acquisition_t* hal_sensors_data, HAL_command_t* hal_sensors_cmd)
{
    ...
    static pixel2_t fileimage[80*120];
    //data handling
    ...
    if (counter %400== 200){
        system("/data/edu/take_image.sh");
    }else if(counter %400==0){
    x_avg = raghav(&fileimage);
    printf("%d\n",counter);
    //printf("x average is %d \n\n\n NEWWW", x_avg);
    }
